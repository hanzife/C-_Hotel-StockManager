...
namespace Product
{
    public partial class Incomes : UserControl
    {
        ...
        public void RemplirDGV_IncomeMonth()
        {
            ...
            d.cmd.CommandText = "select datePayment as 'Date Paiement',Credit_chambre as 'Credit Chambre', Credit_Card as 'Carte Crédit',Cash,Cheque,TotalJour as 'Total Jour'  from EntresMois01 where month(datePayment)= " + DateNow;
            d.cmd.Connection = d.con;
        ...
        {
            string DateNow = DTP_datenow.Value.ToString("MM");
            d.cmd = new SqlCommand("select SUM(Credit_chambre) from EntresMois01 where month(datePayment) =" + DateNow);
            d.cmd.Connection = d.con;
            LBL_Cchambre.Text = d.cmd.ExecuteScalar().ToString();

            d.cmd = new SqlCommand("select SUM(Credit_Card) from EntresMois01 where month(datePayment) =" + DateNow);
            d.cmd.Connection = d.con;
            LBL_Ccarte.Text = d.cmd.ExecuteScalar().ToString();

            d.cmd = new SqlCommand("select SUM(Cash) from EntresMois01 where month(datePayment) =" + DateNow);
            d.cmd.Connection = d.con;
            LBL_cash.Text = d.cmd.ExecuteScalar().ToString();

            d.cmd = new SqlCommand("select SUM(Cheque) from EntresMois01 where month(datePayment) =" + DateNow);
            d.cmd.Connection = d.con;
            LBL_cheque.Text = d.cmd.ExecuteScalar().ToString();

            d.cmd = new SqlCommand("select SUM(TotalJour) from EntresMois01 where month(datePayment) =" + DateNow);
            d.cmd.Connection = d.con;
